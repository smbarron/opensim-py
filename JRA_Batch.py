...
Arguments:
    ...
    not_split_emg_filepath: Path to the non-split EMG data file. This should be a .pkl file containing a dictionary of the form {(subject, trial): emg_data}, 
        where emg_data is a pandas dataframe containing the EMG data for whole trial. The EMG data should be pre-filtered and normalized to maximum contraction values. 
    root_dir: Path to the directory containing the data. The directory structure should be as follows:
        root_dir
        ├── subject_id_1
        │   ├── subject_id_1_clamped.osim or subject_id_1_nosupra.osim (This is an already scaled model with or without the supraspinatus, depending on whether the subject has a tear.)
        │   ├── SO_Results
...
Example Usage:
    python JRA_batch.py osim_emg/tear.txt osim_emg/notear.txt osim_emg/splitnorm_EMG_cleaned.pkl osim_emg/normalized_emg_not_split.pkl test_EMG_package JRA_setup.xml
"""
...
def prep_joint_reactions_analysis(
    ...
    setup_file = create_jra_setup(setup_template, model_file, states_file, split_emg_filepath, not_split_emg_filepath, initial_time, final_time, results_dir, trial_name, cohort)

    return setup_file


def run_joint_reactions_analysis(tear_subjects: List[str], no_tear_subjects: List[str], split_emg_filepath: str, not_split_emg_filepath: str, root_dir: str, setup_template: str):
        ...
        for(_, _, filenames) in os.walk(so_dir):
            for name in filenames:
                if '_states.sto' in name: 
                    ...
                    setup_file = prep_joint_reactions_analysis(setup_template, model_file, states_file, results_dir, split_emg_filepath, not_split_emg_filepath, tool_name, cohort)
...
if __name__ == "__main__":
    if len(sys.argv) != 7:
        logging.info("Usage: python JRA_Batch.py <tear_subjects_file> <no_tear_subjects_file> <split_emg_filepath> <not_split_emg_filepath> <root_dir> <setup_template>")
        sys.exit(1)
